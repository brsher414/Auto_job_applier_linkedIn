...
location = ["Shanghai, China", "Beijing, China"]                      # Top search-bar locations. The bot searches these one by one.
# After Shanghai/Beijing are done, the search rounds below switch to overseas locations.
...
# Search rounds run in order. This keeps Shanghai/Beijing first, then tries overseas locations.
search_rounds = [
    {
...
# Skip jobs only when LinkedIn's visible salary field/card says the monthly salary ceiling is below this amount in CNY.
# Jobs without a visible salary field will not be skipped.
minimum_monthly_salary_cny = 20000

# Avoid applying to these companies, and companies with these bad words in their 'About Company' section...
about_company_bad_words = ["Crossover", "NielsenIQ", "Nielsen IQ", "尼尔森"]       # (dynamic multiple search) or leave empty as []. Ex: ["Staffing", "Recruiting", "Name of Company you don't want to apply to"]
...
bad_words = ["US Citizen", "USA Citizen", "No C2C", "No Corp2Corp", "Security Clearance", ".NET", "Embedded Programming", "PHP", "Ruby", "CNC", "Frontend", "Front End", "Full Stack", "Java Developer", "Sales Representative"]                     # (dynamic multiple search) or leave empty as []. Case Insensitive. Ex: ["word_1", "phrase 1", "word word", "polygraph", "US Citizenship", "Security Clearance"]
